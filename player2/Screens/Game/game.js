...
this.socket.emit('position2', this.position);
...
//SOCKET
  ...
  this.socket.on('tap2', (tap)=>{
    console.log ("recibiendo-tap2:", tap)
    this.fire=tap;
  });

  this.socket.on('position1', (enemy)=>{
...
keyPressed(){
  ...
  if((this.p5.key === 's' || this.p5.key === 'S' || this.p5.keyCode == this.p5.DOWN_ARROW) && this.move===true){
    ...
    if((this.p5.keyCode === 32) && (this.nobullet===true)){
      ...
      this.socket.emit('bullet2', this.mybullet)
